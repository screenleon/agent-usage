...
Windows can expose several `codex.exe` helper processes for one invocation. When their directories cannot be read, agent-usage shows one representative Codex row rather than duplicating the latest thread on every helper process. `tokens_used` is cumulative thread usage; a CTX percentage is shown only when it is within the current model window.
